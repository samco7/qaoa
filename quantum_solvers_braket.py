...
class QAOASolver:
    ...
    def construct_circuit(self):
        ...
        if self.adjust_mixer == False:
            def get_mixer_circuit(parameter):
                for i in self.graph.nodes():
                    circ.rx(i, 2*parameter)
                # return qc
        elif self.warm_start_method in ['GW Rounded', 'BMZ Rounded']:
            def get_mixer_circuit(parameter):
                for i in self.graph.nodes():
                    circ.ry(i, self.thetas[i])
                    circ.rz(i, -2*parameter)
                    circ.ry(i, -self.thetas[i])
                # return qc
        else:
            def get_mixer_circuit(parameter):
        ...
        for i in range(0, self.n_layers):
            beta = FreeParameter(f'beta_{i}')
            gamma = FreeParameter(f'gamma_{i}')
            betas.append(beta)
            gammas.append(gamma)
            get_cost_circuit(gamma)
            get_mixer_circuit(beta)
        # circ.measure_all()
    ...
    def execute_circuit(self, betas, gammas):
        ...
        for i in range(self.n_layers):
            bound_values[f'beta_{i}'] = betas[i]
        for j in range(self.n_layers):
            bound_values[f'gamma_{j}'] = gammas[j]
        bound_circ = self.circ.make_bound_circuit(bound_values)
        self.n_circ_evals += 1
        counts = self.backend.run(bound_circ, shots=self.shots).result().measurement_counts
        return counts
